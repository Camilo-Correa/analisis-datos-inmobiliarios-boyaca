...
def render_tab(tab):
    if tab == 'tab-mapa':
        ...
        return html.Div([
            dcc.Graph(id='grafico-precio-tipo-ciudad')
        ], style={'width': '60%', 'margin': 'auto'})

    elif tab == 'tab-ciudad':
        return html.Div([
            dcc.Graph(id='grafico-cantidad-inmuebles')
        ], style={'width': '60%', 'margin': 'auto'})
...
def actualizar_mapa(ciudad, tipo_inmueble, estrato):
    ...
    if tipo_inmueble:
        df_f = df_f[df_f['tipo_inmueble'] == tipo_inmueble]
    if estrato:
...
def actualizar_barra(ciudad, estrato):
    ...
    fig = px.bar(df_f, x='area m²', y='precio_millones', color='estado_inmueble',
                 ...
                 labels={'area m²': 'Área (m²)', 'precio_millones': 'Precio (Millones COP)'},
...
def actualizar_lotes(ciudad):
    ...
    fig = px.scatter_mapbox(
        ...
        hover_name='ubicacion_asociada', mapbox_style="open-street-map", zoom=12,
        color_continuous_scale='Viridis', title=f"Lotes en {ciudad}"
    )
    fig.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 0})
    return fig
...
@app.callback(Output('grafico-precio-tipo-ciudad', 'figure'), Input('tabs', 'value'))
def actualizar_precio_tipo(tab):
    if tab == 'tab-precio-tipo':
        promedio = df.groupby(['Ciudad', 'tipo_inmueble'])['precio_millones'].mean().reset_index()
        fig = px.bar(promedio, x='Ciudad', y='precio_millones', color='tipo_inmueble', barmode='group',
                     title="Precio Promedio por Tipo de Inmueble en cada Ciudad",
                     labels={'precio_millones': 'Precio promedio (millones COP)'})
        fig.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 0})
        return fig

@app.callback(Output('grafico-cantidad-inmuebles', 'figure'), Input('tabs', 'value'))
def actualizar_inmuebles_ciudad(tab):
    if tab == 'tab-ciudad':
        conteo = df.groupby(['Ciudad', 'tipo_inmueble']).size().reset_index(name='cantidad')
        fig = px.bar(conteo, x='Ciudad', y='cantidad', color='tipo_inmueble', barmode='group',
                     title="Cantidad de Inmuebles por Tipo y Ciudad")
        fig.update_layout(margin={"r": 0, "t": 40, "l": 0, "b": 0})
        return fig
